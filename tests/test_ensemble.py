"""
Test module for ensemble. Execute with pytest
"""
...
def name_fun(index):
    return 'tests/test_img' + str(index) + '.gslib'

def test_Ensemble():
    ensemble = mpstool.ensemble.ensemble_as_subimages(mpstool.img.Image.fromGslib('tests/test_img.gslib'), (2, 2, 1), 2)
    ensemble = mpstool.ensemble.ensemble_from_files(name_fun, mpstool.img.Image.fromGslib, 2)
